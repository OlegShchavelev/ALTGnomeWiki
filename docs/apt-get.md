...
- найти пакеты в репозитории
 ...
 Все операции с пакетами с помощью пакетным менеджем проводятся в консольной строке. Для большинства команд пакетного менеджера нужны права суперпользователя. Хотя, для операции поиска, достаточно обычных прав пользователя. Установка приложение происходит из подключенного репозитория Сизиф(Sisyphus)
