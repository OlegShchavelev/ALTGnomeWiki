...
```
::: info
Для драйверов версии 545 и выше, нет необходимости указывать modeset в параметрах ядра. Эта настройка будет уже правильно установлена вместе с установкой самих драйверов.(Настройка будет установлена в/etc/modprobe.d/nvidia_common.conf)
Если у вас драйвера ниже 545, или же вы решили откатить до более низкой версии, добавляем в параметр `GRUB_CMDLINE_LINUX_DEFAULT` значение `nvidia-drm.modeset=1` и сохраняем:
:::
...
По идее, всех предыдущих этапов было бы достаточно для активации wayland, но в правилах дисплейного менеджера GDM не предусмотрено, что один из важных элементов будет находиться в другом каталоге. Поэтому мы эти [правила](https://gitlab.gnome.org/GNOME/gdm/-/blob/main/data/61-gdm.rules.in#L51) маскируем:
```shell
