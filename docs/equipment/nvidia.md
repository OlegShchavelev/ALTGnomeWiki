...
```
::: tip
Эти интерфейсы заменяя стандартные sleep hibernate и resume, давая возможность записать ресурсы видеопамяти перед отключением питания у видеокарты.
:::
...
```
::: info
Перед маскированием правил, удостоверьтесь, что Вы правильно настроили [сохранение выделенных ресурсов видеопамяти](./nvidia.md#настроика-сохранение-выделенных-ресурсов-видеопамяти). Чаще всего проблемы именно из-за этого.
...
Либо же установить автозапуск команд через CLI [nvidia-settings](./nvidia.md#nvidia-settings), например через `~/.bashrc`.
Пример:
