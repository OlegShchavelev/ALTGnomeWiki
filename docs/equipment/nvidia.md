...
Если вы не используете вышеупомянутые технологии, и ваша видеокарта работает стабильно на Nouveau, рекомендуется следует остаться на нём.
...
#### Сессия Wayland
Драйверы до версии 364.12 **не име поддержку Kernel Mode Settings**.
...
файлов символьных устройств NVIDIA. Эти возможности обычно предоставляся
...
```
::: tip
Эти интерфейсы заменя стандартные sleep hibernate и resume, давая возможность записать ресурсы видеопамяти перед отключением питания у видеокарты.
:::
...
Если система и видеочип поддержива указанные режимы, добавляем параметр  `NVreg_EnableS0ixPowerManagement=1`:
...
```
::: info
Перед маскированием правил, удостоверьтесть, что Вы правильно настроили [сохранение выделенных ресурсов видеопамяти](./nvidia.md#настроика-сохранение-выделенных-ресурсов-видеопамяти). Чаще всего проблемы именно из-за этого.
