...
Предлагаемый [NVIDIA](https://www.nvidia.com/), собственный драйвер для своих продуктов. Этот драйвер являются [закрытым программным обеспечением](https://en.wikipedia.org/wiki/Proprietary_software) и поставляются отдельно от ядра Linux, требуя индивидуальной установки.

Важно отметить, что драйвер NVIDIA постоянно развивается и обновляется, и различные версии этого драйвера могут иметь свои особенности и ограничения, [поэтому критически важно знать версию установленного драйвера и информацию, связанную с ними.](nvidia.md#настроика-драивера)
...
Драйвер Nouveau изначально присутствуют в ядре системы, и настройка не требуется.
...
::: tip
Для  установки во время выполнения скрипта следует читать сообщения из консоли и следовать указанным там шагам. Возможно, потребуется несколько перезагрузок. Например, для автоматического обновления ядра.
:::
...
При установке драйвера через [EPM](./nvidia.md#способ-1-через-epm) настройка под wayland уже будет произведена. Если Вы установили таким способом, дальнейшая настройка не требуется.

Если установили вручную, смотрите дальнейшую настройку.

#### Сессия Wayland
Для работы Wayland необходимо настроить [сохранение выделенных ресурсов видеопамяти](./nvidia.md#настроика-сохранение-выделенных-ресурсов-видеопамяти)

::: warning
У драйвера NVIDIA существуют [известные проблемы и несовместимости](./nvidia.md#проблемы-c-wayland-протоколом), связанные с wayland. Вы можете ознакомится с ними для лучшего понимания дел на данный момент.
:::


### Драйвер 470.223.02
::: info
Драйвера до версии 495 имеют поддержку только одного buffer API — [EGLStreams](https://www.phoronix.com/news/GNOME-Mutter-Mainline-EGLStream).
...
1. Активировать метод [Kernel Mode Setting](./nvidia.md#активация-kernel-mode-setting)
2. Настроить [сохранение выделенных ресурсов видеопамяти](./nvidia.md#настроика-сохранение-выделенных-ресурсов-видеопамяти)
...
### nvidia-smi
[nvidia-smi](https://developer.download.nvidia.com/compute/DCGM/docs/nvidia-smi-367.38.pdf) (также NVSMI) предоставляет возможности мониторинга и управления для каждого из
Устройства NVIDIA Tesla, Quadro, GRID и GeForce семейства Fermi и более высоких архитектур.
...
Необходимо прописать в параметр `GRUB_CMDLINE_LINUX_DEFAULT` значение `nvidia-drm.modeset=1` и cгенерировать новых grub.cfg:
...
::: warning
Существуют проблемы на некоторых устройствах с PRIME. Этот способ направлен больше на **настольные ПК**
:::
...
Драйвер NVIDIA имеют поддержку динамического управления питанием графического процессора NVIDIA ([PCI-Express Runtime D3 (RTD3) Power Management](https://download.nvidia.com/XFree86/Linux-x86_64/550.40.07/README/dynamicpowermanagement.html).
...
Смотрите настройку [фреймбуфера от NVIDIA](./nvidia.md#фреимбуфер-от-nvidia)
...
```
::: info
Перед маскированием правил, удостоверьтесть, что Вы правильно настроили [сохранение сохранение выделенных ресурсов видеопамяти](./nvidia.md#настроика-сохранение-выделенных-ресурсов-видеопамяти). Чаще всего проблемы именно из-за этого.
...
### Не работает KMS
Если при загрузке GNOME чёрный экран, либо при проверке 'cat /sys/module/nvidia_drm/parameters/modeset' выводит 'N', можно попробовать [включить KMS через командную строку ядра](./nvidia.md#активация-kernel-mode-setting)
...
### [nvidia-smi](./nvidia.md#nvidia-smi)
...
Для сессии x11 (через [X Config Options](https://us.download.nvidia.com/XFree86/Linux-x86_64/364.19/README/xconfigoptions.html)) существует возможность активировать неподдерживаемые NVIDIA функции через [NV-CONTROL X](https://www.nvidia.com/en-us/drivers/IO-10753/) расширение. Активация функций происходит через указание битовой макси:

::: danger
Для работы необоходима сессия под X11 и отключённый режим `nvidia_drm_modeset=0`, поэтому, на сегодняшний день эта возможность является устаревшей.

Также, для разных версиё драйвера в документации заявлено разное кол-во функций. в документации для 364.19 их описано 6, а в 550.54.14 их уже 2.
:::

Установить битовую маску можно либо в конфигурационном файле xorg.conf в разделе Device, либо через такие программы, как [nvidia-xconfig](./nvidia.md#nvidia-xconfig) и [EnvyControl](./nvidia.md#envycontrol)
...
Либо же установить автозапуск комманд через CLI [nvidia-settings](./nvidia.md#nvidia-settings), например через `~/.bashrc`.
Пример:
