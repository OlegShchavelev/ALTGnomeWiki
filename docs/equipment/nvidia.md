...
На момент лета 2024 года она всё ещё не внедрена в систему по умолчанию, т.к. ждёт изменения CST тестов, тем не менее её можно запустить, добавив в переменные окружения `ZINK_DEBUG=ioopt`.
