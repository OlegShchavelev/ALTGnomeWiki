...
Указываются они для для каждого типа работы питания следующим образом: `PerfLevelSrc=0x[стратегия для работы от батареи][cтратегия для работы от сети]`.
...
Режимы производительности применяются только при стратегии с фиксированной частотой `22`. Для адаптимной стратегии `33` указание режима не требуется.
...
Если у вас только один тип работы питания, либо определяется только один (обычно от сети - AC, проверьте текущий параметр `GPUPowerSource`), используйте только только тот параметр, который определяется в системе.
