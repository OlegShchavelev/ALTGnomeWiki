...
Данная статья описывает процесс установки и настройки драйверов для графических карт NVIDIA.

На данный момент существует 2 драйвера:
### 1. Проприетарный драйвер от NVIDIA
Данный драйвер разрабатывается самой компанией [NVIDIA](https://www.nvidia.com/) и являются проприетарной разработкой, в следствии чего поставляются отдельно от ядра Linux и требуют отдельной установки (В отличии от открытых драйверов Nouveau, которые уже есть в системе).
...
### 2. Драйвер Nouveau
Он был создан независимыми разработчиками, главным способом, с помощью реверс-инжиниринга. На данный момент разрабатываются под крылом [X.Org Foundation](https://x.org/), размещается на freedesktop.org ([тут](https://nouveau.freedesktop.org/)) и является частью проекта [MESA 3D](https://www.mesa3d.org/). Подробнее смотри [здесь](https://ru.wikipedia.org/wiki/Nouveau).
...
Данные драйвера изначально присутствуют в системе и их установка не требуется.
...
Проприетарные драйверы NVIDIA обеспечивают управление питанием большего числа видеокарт, поддерживают работу CUDA, а также обеспечивают лучшую поддержку игр и графических программ. Это способствует более комфортной работе с операционной системой.
...
У поддержки управления питания на Nouveau были прогрессы и регрессы (см. [PowerManagement](https://nouveau.freedesktop.org/PowerManagement.html)), т.к. разрабатывалась своими силами путём реверс-инжиниринга.
Но видеокарты, начиная с архитектуры Turing, благодаря GSP прошивкам от Nvidia, теперь имеют полную поддержку управления питания.
Ampere и Ada Lovelace по умолчанию имеют это поддержку, когда как для Turing необходимо прописывание параметра в ядре.

Для того, чтобы на Turing заработало управление питанием, необходимо прописать в параметр `GRUB_CMDLINE_LINUX_DEFAULT` значение `nouveau.config=NvGspRm=1` и cгенерировать новых grub.cfg:
