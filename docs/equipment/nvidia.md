...
### Для корректно работы сна или заморозки в режиме wayland, необходимо правильное сохранение выделенных ресурсов видеопамяти.
...
```
После указания опции и режима , тестируем сон и/или заморозку и проверяем, всё ли корректно загрузилось. Если всё хорошо, добавляем настройки на постоянную загрузку:
...
::: warning
Gnome знает только о сохранении выделенных ресурсов в безымянных временных файлах (О способе 1).

Если Вы хотите использовать способ сохранения через режим сохранения питания S0ix, Вам необходимо [замаскировать правила GDM](./nvidia.md#после-настроек-для-сессии-wayland-wayland-в-gdm-не-появляется)
:::
---
...
1. Использование собственного фреймбуфера от NVIDIA:
   Необходимо добавить опцию [fbdev=1 в /etc/modprobe.d/](./nvidia.md#фреимбуфер-от-nvidia-экспериментальная-функция)
