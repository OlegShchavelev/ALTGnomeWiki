...
- Используйте сканеры аудита безопасности, которые могут показать текущее состояние безопасности системы (напр. `lynis`) и настройте регулярные проверки c получением отчётов;
