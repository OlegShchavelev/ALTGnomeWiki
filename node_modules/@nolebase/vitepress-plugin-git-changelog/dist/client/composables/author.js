...
exports.extractAuthorsWithMultiple = extractAuthorsWithMultiple;
exports.findMapAuthorByEmail = findMapAuthorByEmail;
exports.findMapAuthorByName = findMapAuthorByName;
exports.findMapAuthorLink = findMapAuthorLink;
exports.mapCommitAuthors = mapCommitAuthors;
exports.newAvatarForAuthor = newAvatarForAuthor;
...
function findMapAuthorLink(creator) {
  if (!creator.links && !!creator.username) return `https://github.com/${creator.username}`;
  if (typeof creator.links === "string" && !!creator.links) return creator.links;
  if (!Array.isArray(creator.links)) return;
  const priority = ["github", "twitter"];
  for (const p of priority) {
    const matchedEntry = creator.links?.find(l => l.type === p);
    if (matchedEntry) return matchedEntry.link;
  }
  return creator.links?.[0]?.link;
}
const multipleAuthorsRegex = /^ *Co-authored-by: ?([^<]*)<([^>]*)> */gim;
async function extractAuthorsWithMultiple(mapContributors, authorDataMap, c) {
  if (!c.body) return;
  let result;
  ...
  while (result = multipleAuthorsRegex.exec(c.body)) {
    ...
    await mapCommitAuthors(mapContributors, authorDataMap, {
      ...
      author_avatar: await (0, _utils.digestStringAsSHA256)(email.trim())
    });
  }
}
async function newAvatarForAuthor(mappedAuthor, commitAuthor) {
  if (mappedAuthor) {
    if (mappedAuthor.avatar) return mappedAuthor.avatar;
    if (mappedAuthor.username) return `https://github.com/${mappedAuthor.username}.png`;
  }
  if (commitAuthor.author_avatar) return `https://gravatar.com/avatar/${commitAuthor.author_avatar}?d=retro`;
  return `https://gravatar.com/avatar/${await (0, _utils.digestStringAsSHA256)(commitAuthor.author_email)}?d=retro`;
}
async function mapCommitAuthors(mapContributors, map, commitAuthor) {
  ...
  if (targetCreatorByName) {
    ...
    if (foundSocialEntryUrl) url = foundSocialEntryUrl;
