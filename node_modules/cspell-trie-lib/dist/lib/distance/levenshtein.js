...
export function selectNearestWords(word, words, count, maxEdits) {
    const resultsByCost = [];
    const a = word;
    // By prefixing with spaces, no out of bounds checks are necessary.
    const aa = '  ' + a;
    ...
    for (const b of words) {
        ...
        if (dist === undefined || dist > maxEdits)
            continue;
        ++n;
    ...
    function levenshteinDistance(b) {
        const bb = '  ' + b;
        const nB = b.length + 1;
        let j = 0;
        const lw = lastWord;
