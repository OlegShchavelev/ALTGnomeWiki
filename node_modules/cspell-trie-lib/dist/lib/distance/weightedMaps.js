...
export function createWeightMap(...defs) {
    const map = _createWeightMap();
    addDefsToWeightMap(map, defs);
    return map;
}
export function addDefToWeightMap(map, ...defs) {
    return addDefsToWeightMap(map, defs);
}
export function addAdjustment(map, ...adjustments) {
    for (const adj of adjustments) {
        map.adjustments.set(adj.id, adj);
    }
    return map;
}
export function addDefsToWeightMap(map, defs) {
...
export function* splitMapSubstringsIterable(map) {
    let seq = '';
    let mode = 0;
...
export function splitMapSubstrings(map) {
...
export function createWeightCostCalculator(weightMap) {
    return new _WeightCostCalculator(weightMap);
}
class _WeightCostCalculator {
    weightMap;
...
export function prettyPrintReplace(trie, pfx = '', indent = '  ') {
    function* walk() {
...
export function prettyPrintSwap(trie, pfx = '', indent = '  ') {
    function* walk() {
        for (const { a, b, c, p } of walkTrieTrieCost(trie)) {
            const pm = p ? ` + ${p}` : '';
            yield indent + `(${a}) <-> (${b}) = ${c}${pm}`;
